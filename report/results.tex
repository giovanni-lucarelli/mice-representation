...
Table~\ref{tab:mean_rsa_sem} reports the pooled inter-animal consistency (RSA score) computed for each visual area. The mean RSA values, together with their corresponding standard errors of the mean (SEM), quantify the reliability of neural responses across animals within each region.
...
\begin{table}[h]
    ...
    \begin{tabular}{lc}
        ...
        \hline
    \end{tabular}
    \caption{Interanimal consistency score $\pm$ standard error for each visual area.}
    \label{tab:mean_rsa_sem}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{./image/res0.png}
