...
\begin{figure}[H]
    ...
    \vspace{-1em}
    \caption{RSA improvement of two models w.r.t. the untrained model with no diet at inference. In \textbf{blue} the model without dieth both a training and inference time; in \textbf{green} the one with diet both at train and inference time.}
...
\begin{figure}[H]
    ...
    \vspace{-1em}
    \caption{RSA improvement of different different data diet at inference w.r.t. the untrained model with no diet at inference. In \textbf{dark violet} the model with diet at inference, in \textbf{light violet} the one with random diet at inference.}
    \label{fig:random}
\end{figure}
...
\begin{figure}[H]
    ...
    \vspace{-1em}
    \caption{RSA improvement of different models w.r.t. the untrained model with no diet at inference. In different shades of blue the model trained on ImageNet without and with diet at inference. In different shades of green the model trained on ecologicaly transfomed ImageNet without and with diet at inference.}
    \label{fig:inference}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{RSA improvement of different models w.r.t. the untrained model with no diet at inference. In different shades of blue the model trained on ImageNet with different diet at inference; in green the model trained on ecologicaly transfomed ImageNet with diet at inference.}
...
\begin{figure}[H]
    ...
    \vspace{-1em}
    \caption{RSA improvement of different models w.r.t. the untrained model with no diet at inference. In different shades of blue the model trained on ImageNet with different diet at inference: no diet, diet, diet with no noise, Nayebi diet. In green the model trained on ecologicaly transfomed ImageNet with diet at inference.}
    \label{fig:nayebi}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{./image/ex-nodiet-diet.jpeg}
    \includegraphics[width=0.6\textwidth]{./image/ex-nonoise-nayebi.jpeg}
    \caption{Different data diet transformation on one Allen Barain Dataset stimulus. From top left to bottom right: no diet, diet, diet with no noise, Nayebi diet.}
\end{figure}